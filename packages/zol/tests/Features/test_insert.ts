...
import { booleanCol, ConflictTarget, defaultValue, e, insert, insertMany, insertManyOnConflictDoNothing, insertManyReturning, insertOnConflictDoNothing, insertOnConflictDoUpdate, insertReturning, isNotNull, nullCol, numberCol, Order, order, pg, query, select, textCol, inQuery, restrictEq } from "../../src/zol";
...
test("insert on conflict do update where", t => withTestDatabase(async conn => {
    await pg.query_(conn, createBookTableSql);
...
test("insert on conflict do update where2", t => withTestDatabase(async conn => {
    await pg.query_(conn, createBookTableSql);
    ...
    const inserted2 = await insertOnConflictDoUpdate("", conn,
        ...
        vals2,
        ConflictTarget.tableColumns(["serial"]),
        row => e(isNotNull(row.numPages), "AND", e(row.author, "!=", textCol("Y"))),
        (row) => {
            const newRow: BookTable = {
                ...row,
                numPages: nullCol()
...
test.only("insert on conflict do update where3", t => withTestDatabase(async conn => {
    await pg.query_(conn, createBookTableSql);
    ...
    const inserted2 = await insertOnConflictDoUpdate("", conn,
        ...
        vals2,
        ConflictTarget.tableColumns(["serial"]),
        row => e(isNotNull(row.numPages), "AND", inQuery(textCol("x"), q => {
            const book = select(q, bookTable);
            restrictEq(q, book.author, row.author);
            return book.author;
        })),
        (row) => {
            const newRow: BookTable = {
...
test("insert on conflict do update set cols", t => withTestDatabase(async conn => {
    await pg.query_(conn, createBookTableSql);
    ...
    const inserted2 = await insertOnConflictDoUpdate("", conn,
        ...
        (row) => {
            const newRow: BookTable = {
                ...row,
                title: e(e(row.title, "||", textCol(" (By ")), "||", e(row.author, "||", textCol(")")))
            };
            return newRow;
        }
    );
...

    {
        ...
        const expected_rows: typeof rows = [
            { author: "noone", numPages: null, serial: 5, title: "A book" },
            { author: "X", numPages: 100, serial: 6, title: "Another book (By X)" }
        ];
