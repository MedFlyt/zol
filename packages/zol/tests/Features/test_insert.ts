...
import { booleanCol, ConflictTarget, defaultValue, e, inQuery, insert, insertMany, insertManyOnConflictDoNothing, insertManyReturning, insertOnConflictDoNothing, insertOnConflictDoUpdate, insertReturning, isNotNull, nullCol, numberCol, Order, order, pg, query, restrictEq, select, textCol } from "../../src/zol";
...
test("insert on conflict do update where3", t => withTestDatabase(async conn => {
    await pg.query_(conn, createBookTableSql);
    ...
    const inserted2 = await insertOnConflictDoUpdate("", conn,
        ...
        vals2,
        ConflictTarget.tableColumns(["serial"]),
        row => e(isNotNull(row.numPages), "AND", inQuery(row.author, q => {
            const book = select(q, bookTable);
