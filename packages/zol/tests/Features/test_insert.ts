...
import { booleanCol, ConflictTarget, defaultValue, e, insert, insertMany, insertManyOnConflictDoNothing, insertManyReturning, insertOnConflictDoNothing, insertOnConflictDoUpdate, insertReturning, isNotNull, nullCol, numberCol, Order, order, pg, query, select, textCol } from "../../src/zol";
...
test.only("insert on conflict do update set cols", t => withTestDatabase(async conn => {
    await pg.query_(conn, createBookTableSql);
